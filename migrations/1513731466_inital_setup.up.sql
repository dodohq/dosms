...
(provider_id, start_time, end_time, deleted),
  FOREIGN KEY
(provider_id) REFERENCES providers
(id)
);
CREATE TABLE
...
SET
  deleted = NEW.deleted
WHERE
  provider_id = NEW.id;RETURN NEW;
END;$BODY$
LANGUAGE plpgsql;
CREATE TRIGGER trigger_delete_provider BEFORE
UPDATE
  ON providers FOR EACH ROW
WHEN
(
    OLD.deleted IS DISTINCT
    FROM
      NEW.deleted
  )
...
SET
  deleted = NEW.deleted
WHERE
  time_slot_id = NEW.id;RETURN NEW;
END;$BODY$
LANGUAGE plpgsql;
CREATE TRIGGER trigger_delete_time_slot BEFORE
UPDATE
  ON time_slots FOR EACH ROW
WHEN
(
    OLD.deleted IS DISTINCT
    FROM
...
SET
  deleted = NEW.deleted
WHERE
  orders = NEW.id;RETURN NEW;
END;$BODY$
LANGUAGE plpgsql;
CREATE TRIGGER trigger_delete_order BEFORE
UPDATE
